...
@interface SerialLoginAlertView ()<UIScrollViewDelegate>
...
@implementation SerialLoginAlertView
...
-(void)dl_setupFrameNumber
{
    self.contentViewWidth  = (KScreenwidth * contentViewWidthRatio);
    self.contentViewHeight =  (self.contentViewWidth / contentViewWidhtHeightRatio);
    self.contentViewX =  (KScreenwidth - self.contentViewWidth)/2;
    self.closeButtonWidthHeight = 40;
    self.closeButtonX = (KScreenwidth - self.closeButtonWidthHeight) / 2;
    self.textViewMargin = 10;
    self.marginWidth = 20;
    self.subViewSpaceWidth = (KScreenheight - self.marginWidth - self.contentViewHeight - self.closeButtonWidthHeight) / 2;
}
...
- (void)setTextLable:(NSString *)textString{
    ...
    self.textlable.text = textString;
}
...
-(void)dl_subViewsShowAnimation
{
    ...
    [UIView animateWithDuration:0.3 delay:0.0 usingSpringWithDamping:0.5 initialSpringVelocity:5.0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        self.closeButton.frame = CGRectMake(self.closeButtonX, KScreenheight - self.subViewSpaceWidth - self.closeButtonWidthHeight - 100, self.closeButtonWidthHeight, self.closeButtonWidthHeight);
    } completion:^(BOOL finished) {
...
-(void)dl_subViewsCloseAnimationWithcompletion:(Completion)completion
{
    ...
    CGFloat closeButtonY = KScreenheight;
...
-(void)dl_setupSubViewsFrame
{
    ...
    if (self.imageView) {
        ...
        self.contentView.frame = CGRectMake(self.contentViewX, -contentViewY, self.contentViewWidth, self.contentViewHeight);
        self.imageView.frame =  CGRectMake((self.contentViewWidth - self.imageView.image.size.width)/2 , (self.contentViewHeight - self.imageView.image.size.height)/2, self.imageView.image.size.width, self.imageView.image.size.height);
        self.textlable.frame = CGRectMake(0, (self.contentViewHeight - 100)/2, self.contentView.frame.size.width, 100);
